...
public class TS {
    ...
    public static double MOTOR_REV_SPEED_SEC_ms = 60. * 3600. / 1000.; // 毎秒60度
    private static int INTERVAL_ms = 30;
    ...
    protected BooleanProperty isMeasuring;     // 測距中
    ...
    private TS() {
        isMeasuring = new SimpleBooleanProperty(false);
...
//
    ...
    public BooleanProperty getIsMeasuringProperty() {
        return isMeasuring;
    }
...
//        angleHDMS.set(AngleData.RAD2DMS(Math.atan2(dy, dx)));
        Task task = new Rotate(AngleData.RAD2DMS(Math.atan2(dy, dx)), AngleData.RAD2DMS(Math.atan2(Math.sqrt(Math.pow(dx, 2) + Math.pow(dy, 2)), dz)));
    ...
    public class Rotate extends Task<Void> {
        private final double targetAngleH_SEC;
        private final double targetAngleV_SEC;
        private final double currentH_SEC;
        private final double currentV_SEC;

        public Rotate(double targetAngleHDMS, double targetAngleVDMS) {
            this.targetAngleH_SEC = AngleData.DMS2SEC(targetAngleHDMS);
            this.targetAngleV_SEC = AngleData.DMS2SEC(targetAngleVDMS);
            this.currentH_SEC = AngleData.DMS2SEC(angleHDMS.get());
            this.currentV_SEC = AngleData.DMS2SEC(angleVDMS.get());
        }

        @Override
        protected Void call() throws Exception {

            double dH_SEC = currentH_SEC - targetAngleH_SEC;
            while (dH_SEC > 3600 * 180) dH_SEC -= 3600 * 360;
            while (dH_SEC <= -3600 * 180) dH_SEC += 3600 * 360;
            
            double dV_SEC = currentV_SEC - targetAngleV_SEC;
            while (dV_SEC > 3600 * 180) dV_SEC -= 3600 * 360;
            while (dV_SEC <= -3600 * 180) dV_SEC += 3600 * 360;
//            LOGGER.debug(String.format("dH_DEG = %f, dV_DEG = %f", dH_SEC / 3600., dV_SEC / 3600.));
            boolean doneH = false;
            boolean doneV = false;
            while ((!doneH || !doneV) && !isCancelled()) {
                if (Math.abs(dH_SEC) > MOTOR_REV_SPEED_SEC_ms * INTERVAL_ms) {
                    dH_SEC -= Math.signum(dH_SEC) * MOTOR_REV_SPEED_SEC_ms * INTERVAL_ms;
                } else {
                    dH_SEC = 0.;
                    doneH = true;
                }
                if (Math.abs(dV_SEC) > MOTOR_REV_SPEED_SEC_ms * INTERVAL_ms) {
                    dV_SEC -= Math.signum(dV_SEC) * MOTOR_REV_SPEED_SEC_ms * INTERVAL_ms;
                } else {
                    dV_SEC = 0.;
                    doneV = true;
                }
//                LOGGER.debug(String.format("dH_DEG = %f, dV_DEG = %f", dH_SEC / 3600., dV_SEC / 3600.));
                updateCurrent(AngleData.SEC2DMS(targetAngleH_SEC + dH_SEC), AngleData.SEC2DMS(targetAngleV_SEC + dV_SEC));
                sleep(INTERVAL_ms);
            }
            return null;
        }
