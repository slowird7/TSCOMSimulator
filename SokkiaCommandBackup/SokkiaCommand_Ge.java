...
public class SokkiaCommand_Ge extends SokkiaCommand {

    private final static Logger logger = LogManager.getLogger(SokkiaCommand_Ge.class);

    private final String SEND_COMMAND = "Ge";

    public SokkiaCommand_Ge() {
...
//		}
//
//		Command comm = new SokkiaCommand_Ge();
//		try {
//			comm.sendCommand();
...
//		}
    ...
    public double[] getCoordinate() throws ReceiveFailedException {
        ...
        try {
            ...
            double[] retD = {Double.parseDouble(tmp[3]), Double.parseDouble(tmp[4]), Double.parseDouble(tmp[5])};
            return retD;
        } catch (Exception e) {
            logger.error("Error Message", e);
            throw new ReceiveFailedException();
     ...
     * @throws ReceiveFailedException
     */
    public double[] getDifference() throws ReceiveFailedException {
        int count = 0;
        while (true) {
            if (!receive.equals("")) {
                break;
