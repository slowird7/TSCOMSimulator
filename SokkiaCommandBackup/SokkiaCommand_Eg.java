...
/**
 * 2回sendすると前回と今回の測点間の水平距離が返ってくる。
 */
public class SokkiaCommand_Eg extends SokkiaCommand {

    private final static Logger logger = LogManager.getLogger(SokkiaCommand_Eg.class);

    private final String SEND_COMMAND = "Eg";

    public SokkiaCommand_Eg() {
...
//				e.printStackTrace();
//			}
//		Command comm = new SokkiaCommand_Eg();
//		try {
//			comm.sendCommand();
...
//		}
    ...
    public double getDistance() throws ReceiveFailedException {
        ...
        try {
            String ret = receive;
            ret = ret.replaceAll("Eg ", "");
            String[] tmp = ret.split(",");

            return Double.parseDouble(tmp[4]);	// 測点間水平距離
        } catch (Exception e) {
            logger.error("Error Message", e);
            throw new ReceiveFailedException();
        }
